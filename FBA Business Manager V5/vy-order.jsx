...
function VyOrderJourney({ onJump }) {
  ...
  // Inspection can be turned off per order — keep it in the journey but mark it
  // "skipped" (greyed, dashed) rather than dropping it, so the flow stays legible.
  const inspectionOn = (typeof ordInspectionRequired === "function") ? ordInspectionRequired(ORDER.id) : true;
  const journeyRows = SECTION_MAP_ROWS;
  const cols = [];
