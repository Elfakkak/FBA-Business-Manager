...
export default async function FbaShipmentsPage() {
  ...
  const { data: inbounds } = await supabase
    .from("fba_inbounds")
    .select("id, fc, sku_count, expected, received, amazon_status, synced, eta, eta_from, eta_to, mode, shipment_id, order_id, reference_id, fba_inbound_items(sku, fnsku, expected, received)")
  ...
  type Row = {
    ...
    synced: string | null; eta: string | null; eta_from: string | null; eta_to: string | null; mode: string | null; shipment_id: string | null; order_id: string | null; reference_id: string | null;
