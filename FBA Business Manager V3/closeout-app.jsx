...
const CO_SKUS = CO_SCOPE ? CO_SCOPE.skus.map((s) => ({ sku: s.sku, name: s.name, qty: s.qty, goods: (CO_BILLED[s.sku] != null ? CO_BILLED[s.sku] : s.line), goodsBilled: CO_BILLED[s.sku] != null, salePrice: coCatalogPrice(s.sku) })) : [
      ...
      <section className="vy-card" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ display: "flex", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 460px", padding: "20px 22px", minWidth: 0 }}>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>Landed cost</h1>
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.sku} style={{ borderTop: "1px solid hsl(var(--border))" }}>
                    <td style={coTd}>
                      <div style={{ fontFamily: coMono, fontWeight: 600, fontSize: 12 }}>{r.sku}</div>
                      <div style={{ fontSize: 11, color: "hsl(var(--muted-fg))", marginTop: 1 }}>{r.name}</div>
                    </td>
