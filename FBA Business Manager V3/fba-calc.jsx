...
function FbaCalculator({ compact, prefill }) {
  ...
  const m = useFcMemo(() => {
    ...
    const dims = [fcNum(dimL), fcNum(dimW), fcNum(dimH)];
    const ref = fcNum(referralPct) / 100;
    const tier = fcSizeTier(weight, dims);
    const fbaFee = fcFulfillmentFee(weight, dims);
    ...
    return {
      ...
      hasDims: dims.some((x) => x > 0),
      totalProfit: net * u, totalRevenue: price * u, totalLanded: landed * u,
      verdict: fcVerdict(net, marginPct, roiPct),
    };
  }, [salePrice, costMode, unitCost, freightUnit, dutyPct, landedSingle, dimL, dimW, dimH, weight, referralPct, months, peak, units]);
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: compact ? "1fr" : "minmax(0, 1fr) minmax(0, 0.92fr)", gap: 18, alignItems: "start" }}>
      {/* ---------------- INPUTS ---------------- */}
      <div className="vy-card" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 18 }}>
        {fams.length ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <FcField label="Length" flex="1 1 80px"><FcNum value={dimL} onChange={setDimL} suffix="in" step="0.1" /></FcField>
            <FcField label="Width" flex="1 1 80px"><FcNum value={dimW} onChange={setDimW} suffix="in" step="0.1" /></FcField>
            <FcField label="Height" flex="1 1 80px"><FcNum value={dimH} onChange={setDimH} suffix="in" step="0.1" /></FcField>
            <FcField label="Weight" flex="1 1 90px"><FcNum value={weight} onChange={setWeight} suffix="lb" step="0.05" /></FcField>
          </div>
      ...
      {/* ---------------- RESULTS ---------------- */}
      <div style={{ display: "flex", flexDirection: "column", gap: 14, position: compact ? "static" : "sticky", top: 16 }}>
        {/* Headline */}
...
function FbaCalcModalApp() {
  ...
  return ReactDOM.createPortal(
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }} style={{ position: "fixed", inset: 0, zIndex: 9997, background: "hsl(0 0% 0% / 0.5)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "5vh 16px 16px", overflowY: "auto" }}>
      <div className="vy-card" style={{ width: "100%", maxWidth: 880, padding: 0, boxShadow: "var(--shadow-lg)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "16px 20px", borderBottom: "1px solid hsl(var(--border))" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>FBA calculator</h3>
              <p style={{ margin: "1px 0 0", fontSize: 11.5, color: "hsl(var(--muted-fg))" }}>Should you buy it? — margin, ROI &amp; breakeven before a PO</p>
            </div>
          </div>
