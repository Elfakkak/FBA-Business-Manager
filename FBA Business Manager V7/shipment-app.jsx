          ...
          <div className="vy-content-inner">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 16, alignItems: "start" }} className="sh-two-col">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {/* Ship from / Ship to */}
                {(() => {
                  const a = (typeof logShipmentAddresses === "function") ? logShipmentAddresses(s) : null;
                  if (!a) return null;
                  ...
                  return (
                    <ShCard icon="mapPin" title="Ship from / Ship to" iconTone="muted-fg">
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 12.5, fontWeight: 600 }}>{a.to.place}</div>
                          <div style={{ fontSize: 12, color: "hsl(var(--muted-fg))", lineHeight: 1.5 }}>
                            {a.to.fcs.length ? "Onward to " + a.to.fcs.join(" · ") : "Destination port / hub"}
                          </div>
                        </div>
                      </div>
                    </ShCard>
            ...
            {(() => {
              ...
              return (
                <ShCard icon="shield" iconTone={r.tone === "muted" ? "primary" : r.tone} title="Customs & duties" sub="Who clears customs and pays import duties — decided by the incoterm.">
                  ...
                  {r.needsBroker ? (
                    <>
                      <div style={{ fontSize: 11.5, color: "hsl(var(--muted-fg))", margin: "14px 0 8px", lineHeight: 1.45 }}>
                        <strong style={{ fontWeight: 600, color: "hsl(var(--foreground))" }}>You're the importer of record.</strong> Appoint a customs broker and record the duties &amp; taxes you pay — they add to this order's landed cost.
                      </div>
                      <ShGrid cols={2}>
                        <ShField label="Customs broker" value={s.broker || "— (recommended)"} editing={editing} name="broker" form={form} setForm={setForm} />
                        <ShField label="Duties &amp; taxes (USD)" value={s.dutiesUsd ? shMoney(s.dutiesUsd) : "—"} mono editing={editing} name="dutiesUsd" form={form} setForm={setForm} type="number" />
                      </ShGrid>
                    </>
                  ) : (
                    <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginTop: 14, padding: "11px 13px", borderRadius: 10, background: "hsl(var(--muted-bg) / 0.5)", border: "1px solid hsl(var(--border) / 0.7)" }}>
                      <VyIcon name="info" size={14} style={{ color: "hsl(var(--muted-fg))", flexShrink: 0, marginTop: 1 }} />
                      <div style={{ fontSize: 11.5, color: "hsl(var(--muted-fg))", lineHeight: 1.45 }}>
                        <strong style={{ fontWeight: 600, color: "hsl(var(--foreground))" }}>Nothing to enter under {s.incoterm}.</strong> Your forwarder clears customs and pays the duties &amp; taxes — they're already baked into the freight price, so there's no broker for you to appoint and no duty bill for you to record.
                      </div>
                    </div>
                  )}
                </ShCard>
