...
function VyActivityDrawer({ open, onClose }) {
  const [filter, setFilter] = vyUseState("All");
  // Hide inspection from the journal when this order skips inspection.
  const inspReq = (typeof ordInspectionRequired === "function" && typeof ORDER !== "undefined" && ORDER) ? ordInspectionRequired(ORDER.id) : true;
  vyUseEffect(() => {
    if (!open) return;
  ...
  // If the active filter is one we just hid, fall back to All.
  vyUseEffect(() => { if (!inspReq && filter === "Insp") setFilter("All"); }, [inspReq, filter]);
    ...
    <React.Fragment>
      ...
      >
        ...
        <div className="vy-drawer-filters" role="tablist">
          {["All", "Pay", "Inv", "Insp", "Ship", "Doc"].filter((c) => c !== "Insp" || inspReq).map((c) => (
            <button key={c} type="button" role="tab" aria-selected={filter === c} className={"vy-drawer-chip" + (filter === c ? " is-active" : "")} onClick={() => setFilter(c)}>
              {c}
            </button>
          ))}
        </div>
        <div className="vy-drawer-body">
          {(() => {
            const groups = ACTIVITY_FEED
              .map((group) => ({ ...group, items: group.items.filter((it) => (filter === "All" || it.src === filter) && (inspReq || it.src !== "Insp")) }))
              .filter((group) => group.items.length);
            if (!groups.length) {
              return <div style={{ padding: "32px 16px", textAlign: "center", fontSize: 12.5, color: "hsl(var(--muted-fg))" }}>No {filter} activity on this order yet.</div>;
            }
        ...
        <div className="vy-drawer-foot">
          <a href={"Vyonix Activity.html" + (filter !== "All" ? "?cat=" + filter : "")}>
            See full journal
