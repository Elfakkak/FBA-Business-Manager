          ...
          <div>
            ...
            {hasTracking ? (
              <>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 14 }}>
                  ...
                  <button type="button" className="vy-btn vy-btn--ghost vy-btn--sm" onClick={() => setPasteOpen(true)}>
                    <VyIcon name="pencil" size={13} /><span>Edit tracking</span>
                  </button>
                  {!atEnd ? (
              ...
              <div style={{ padding: "16px", borderRadius: 10, border: "1px dashed hsl(var(--border))", textAlign: "center" }}>
                <div style={{ fontSize: 12.5, color: "hsl(var(--muted-fg))", marginBottom: 10 }}>No tracking number yet. Paste the forwarder's message or type it in.</div>
                <button type="button" className="vy-btn vy-btn--primary vy-btn--sm" onClick={() => setPasteOpen(true)}>
                  <VyIcon name="plus" size={13} /><span>Add tracking</span>
                </button>
              </div>
            )}
          </div>
...
function ShpPasteModal({ shipment: s, onClose, onApplied }) {
  const info0 = (typeof trkInfo === "function") ? trkInfo(s) : {};
  const [text, setText] = useShpState("");
  const [form, setForm] = useShpState(() => ({
    trackingNo: info0.trackingNo || "",
    bookingRef: info0.bookingRef || "",
    carrier: info0.carrier || "",
    eta: (s.eta && s.eta !== "—") ? s.eta : "",
  }));
  const [touched, setTouched] = useShpState(false);
  const set = (k) => (e) => { setTouched(true); setForm((p) => ({ ...p, [k]: e.target.value })); };
  ...
  // When the parser finds new values, push them into the editable form (only
  // filling fields the user hasn't manually overridden in this paste session).
  useShpEffect(() => {
    if (!parsed) return;
    setForm((p) => ({
  ...
  function apply() {
    trkApplyManual(s, { trackingNo: form.trackingNo, bookingRef: form.bookingRef, carrier: form.carrier, eta: form.eta });
    onApplied();
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "hsl(0 0% 0% / 0.5)", display: "grid", placeItems: "center", zIndex: 10000, padding: 20 }} onClick={onClose}>
      <div className="vy-card" style={{ width: "100%", maxWidth: 540, padding: 0, boxShadow: "var(--shadow-lg)", maxHeight: "90vh", display: "flex", flexDirection: "column" }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "20px 24px 14px", borderBottom: "1px solid hsl(var(--border))" }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>Add tracking</h3>
            <p style={{ fontSize: 12.5, color: "hsl(var(--muted-fg))", margin: "4px 0 0", maxWidth: "46ch" }}>Paste the forwarder's message to auto-fill, or type the details in directly. Everything below is editable before you save.</p>
          </div>
        ...
        <div style={{ padding: "18px 24px", overflowY: "auto" }}>
          <div style={fieldWrap}>
            ...
            <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Paste the forwarder's booking / tracking email here — Vyonix pulls out the number, ref and ETA…"
              style={{ ...fieldInput, height: 96, lineHeight: 1.5, resize: "vertical" }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={{ ...fieldWrap, gridColumn: "1 / -1" }}>
              ...
              <input value={form.trackingNo} onChange={set("trackingNo")} autoFocus placeholder="e.g. 6156743593" style={{ ...fieldInput, ...shpMono, fontWeight: 700 }} />
            </div>
            <div style={fieldWrap}>
              <span style={fieldLabel}>Booking ref</span>
              <input value={form.bookingRef} onChange={set("bookingRef")} placeholder="optional" style={{ ...fieldInput, ...shpMono }} />
            </div>
            <div style={fieldWrap}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, padding: "14px 24px", borderTop: "1px solid hsl(var(--border))" }}>
          <button type="button" className="vy-btn vy-btn--ghost" onClick={onClose}>Cancel</button>
          <button type="button" className="vy-btn vy-btn--primary" disabled={!canApply} style={canApply ? undefined : { opacity: 0.5, cursor: "not-allowed" }} onClick={apply}>
