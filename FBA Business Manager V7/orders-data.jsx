...
function ordAllOrders() {
  ...
  return [...drafts, ...ORDERS_LIST.filter((o) => !seen.has(o.id))].map((o) => {
    ...
    return { ...row, archived: ordIsArchived(o.id) };
  });
}

// ----------------------------------------------------------------------
// Order ARCHIVE (persisted). Archived orders are hidden from the default
// Orders list but kept (reversible) — surfaced via the "Show archived" toggle.
// ----------------------------------------------------------------------
const ORD_ARCHIVE_KEY = "vy_order_archived_v1";
function ordArchivedSet() {
  try { const a = JSON.parse(localStorage.getItem(ORD_ARCHIVE_KEY) || "[]"); return new Set(Array.isArray(a) ? a : []); }
  catch (e) { return new Set(); }
}
function ordIsArchived(id) { return ordArchivedSet().has(id); }
function ordSetArchived(ids, archived) {
  const set = ordArchivedSet();
  (Array.isArray(ids) ? ids : [ids]).forEach((id) => { if (archived) set.add(id); else set.delete(id); });
  try { localStorage.setItem(ORD_ARCHIVE_KEY, JSON.stringify([...set])); } catch (e) {}
}
...
Object.assign(window, {
  ...
  ordArchivedSet, ordIsArchived, ordSetArchived,
