    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9998 }}>
      ...
      <aside style={{ position: "absolute", top: 0, right: 0, height: "100%", width: "min(460px, 94vw)", background: "hsl(var(--card))", borderLeft: "1px solid hsl(var(--border))", boxShadow: "var(--shadow-lg)", display: "flex", flexDirection: "column", transform: shown ? "translateX(0)" : "translateX(100%)", transition: "transform 240ms cubic-bezier(0.32,0.72,0,1)" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 20px", display: "flex", flexDirection: "column", gap: 22 }}>
          ...
          {/* Order & cost history (compact) — full detail on the variant page */}
          {(() => {
            const hist = (typeof varHistory === "function") ? varHistory(v) : null;
            if (!hist || !hist.runs.length) return null;
            const T = hist.totals;
            return (
              <div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 8 }}>
                  <div className="vy-kicker">Order &amp; cost history</div>
                  <span style={{ fontSize: 10.5, color: "hsl(var(--muted-fg))" }}>{T.runCount} runs · avg landed {pmoney(T.avgLanded)}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 1, border: "1px solid hsl(var(--border))", borderRadius: 10, overflow: "hidden" }}>
                  {hist.runs.slice().reverse().slice(0, 3).map((r, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 10, alignItems: "center", padding: "9px 12px", background: r.current ? "hsl(var(--primary) / 0.05)" : "hsl(var(--card))", borderTop: i ? "1px solid hsl(var(--border) / 0.6)" : "none" }}>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 600 }}>{r.date}{r.current ? <span className="vy-badge vy-badge--info" style={{ marginLeft: 6, fontSize: 9 }}>latest</span> : null}</div>
                        <div style={{ ...mono, fontSize: 10.5, color: "hsl(var(--muted-fg))" }}>{r.units.toLocaleString()} units · {pmoney(r.unitBilled)}/u billed</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div className="vy-kicker" style={{ marginBottom: 1 }}>Landed/u</div>
                        <div style={{ ...mono, fontSize: 12.5, fontWeight: 700 }}>{pmoney(r.landedUnit)}</div>
                      </div>
                      <div style={{ textAlign: "right", minWidth: 44 }}>
                        <div className="vy-kicker" style={{ marginBottom: 1 }}>Sold</div>
                        <span className={"vy-badge vy-badge--" + (r.sellThrough >= 85 ? "success" : r.sellThrough >= 60 ? "warning" : "muted")}>{r.sellThrough}%</span>
                      </div>
                    </div>
                  ))}
                </div>
                <a href={"Vyonix Variant.html?sku=" + encodeURIComponent(v.sku)} style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 10, fontSize: 12, fontWeight: 600, color: "hsl(var(--primary))", textDecoration: "none" }}>
                  Open full SKU page<VyIcon name="arrowUpRight" size={12} />
                </a>
              </div>
            );
          })()}
        ...
        {confirmDel ? (
          ...
          <div style={{ display: "flex", gap: 10, padding: "14px 20px", borderTop: "1px solid hsl(var(--border))", alignItems: "center" }}>
            ...
            <div style={{ flex: 1 }} />
            <a href={"Vyonix Variant.html?sku=" + encodeURIComponent(v.sku)} className="vy-btn vy-btn--outline" style={{ textDecoration: "none" }}>
              <VyIcon name="arrowUpRight" size={14} /><span>Full page</span>
            </a>
                      ...
                      <tr key={v.sku} className="vy-order-row" onClick={() => setDrawerSku(v.sku)} style={{ borderTop: "1px solid hsl(var(--border))", cursor: "pointer" }}>
                        <td style={prodTd}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <image-slot id={"pvar-" + v.sku} style={{ width: "30px", height: "30px", flexShrink: 0 }} shape="rounded" radius="7" placeholder={v.sku}></image-slot>
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)", fontWeight: 700, fontSize: 12, whiteSpace: "nowrap" }}>{v.sku}</div>
                              <div style={{ fontSize: 11, color: "hsl(var(--muted-fg))", whiteSpace: "nowrap" }}>{variantLabel(v)}</div>
                            </div>
                          </div>
                        </td>
                        <td style={{ ...prodTd }}>
