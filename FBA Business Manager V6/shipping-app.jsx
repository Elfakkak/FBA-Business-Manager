...
function VyShippingBody() {
  ...
  function handleApplyTracking(parsed) {
    const patch = {};
    if (parsed.trackingNo) patch.tracking = parsed.trackingNo;
    if (parsed.etaDate) {
      patch.eta = parsed.etaDate.toLocaleDateString(undefined, { month: "short", day: "numeric" });
      patch.etaShort = patch.eta;
    }
    if (parsed.bookingRef && (!shipment.tracking || shipment.tracking === "—")) patch.tracking = patch.tracking || parsed.bookingRef;
...
function ShipPasteModal({ shipment, onClose, onApply }) {
  ...
  const row = (label, val) => (
    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "7px 0", borderTop: "1px solid hsl(var(--border))" }}>
      ...
      <span style={{ fontFamily: "var(--font-mono, monospace)", fontSize: 12, fontWeight: val ? 700 : 400, color: val ? "hsl(var(--foreground))" : "hsl(var(--muted-fg))" }}>{val || "not found"}</span>
    </div>
  );

  return (
    <ShipModalShell
      title="Paste forwarder update"
      sub="Paste the booking / tracking message (CN or EN). Vyonix pulls out the tracking number and ETA for this shipment."
      onClose={onClose}
      width={540}
      footer={
        <>
          <button type="button" className="vy-btn vy-btn--ghost" onClick={onClose}>Cancel</button>
          <button type="button" className="vy-btn vy-btn--primary" disabled={!found} style={found ? undefined : { opacity: 0.5, cursor: "not-allowed" }} onClick={() => onApply(parsed)}>
