...
function FbaCalculator({ compact, prefill }) {
  ...
  // "muted" is not a color token (only --muted-fg/--muted-bg exist), so map it to a
  // valid one for inline hsl() — otherwise the headline card's tint/border don't render.
  ...
  const Row = ({ label, value, minus, strong, tone, sub }) => (
    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, padding: strong ? "12px 0 2px" : "8px 0", borderTop: strong ? "1.5px solid hsl(var(--border))" : "1px solid hsl(var(--border) / 0.5)" }}>
      ...
      <span style={{ fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)", fontVariantNumeric: "tabular-nums", fontSize: strong ? 18 : 13.5, fontWeight: strong ? 800 : 600, color: tone ? "hsl(var(--" + tone + "))" : (strong ? "hsl(var(--" + vtc + "))" : "hsl(var(--foreground))"), whiteSpace: "nowrap" }}>{minus && value > 0 ? "−" : ""}{fcMoney(value)}</span>
    </div>
  );
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 340px), 1fr))", gap: 16, alignItems: "start" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, position: "sticky", top: 4 }}>
        {/* Headline */}
        <div className="vy-card" style={{ padding: "20px 22px", background: "hsl(var(--" + vtc + ") / 0.06)", border: "1px solid hsl(var(--" + vtc + ") / 0.28)" }}>
          ...
          <div style={{ fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)", fontSize: 40, fontWeight: 800, lineHeight: 1.05, color: "hsl(var(--" + vtc + "))", marginTop: 6, fontVariantNumeric: "tabular-nums" }}>
