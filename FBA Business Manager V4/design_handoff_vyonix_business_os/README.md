...
> **Start with `BUILD_PROMPT.md`** — a paste-ready generator prompt for Claude Code that frames the whole app as components, defines the **motion system** (so animation is consistent app-wide), and gives an execution order. This README is the reference spec behind it; `PROJECT_ARCHITECTURE.md` is the full file map + change log.
...
A restrained, consistent motion language (full spec + tokens in `BUILD_PROMPT.md`). Defined in `vy-app.css`:
- **Signature easing** `cubic-bezier(0.16, 1, 0.3, 1)` for entrances/transforms; plain `ease` for color/hover/state.
- **Duration scale**: 120ms (hover/color/focus) · 160–180ms (menus, pop-ins, row-lift) · 240–280ms (page enter, scrims, dialogs, sidebar width) · 320ms (right drawers) · 400ms (progress fills).
- **Named keyframes**: `vy-page-in` (page/section opacity enter), `vy-fade-down` (nav submenu), `vy-pop-in` (pop-over menu), `vgsPop` (quick-create palette, **transform-only** so it never gets stuck invisible).
- **Transitions** (not keyframes) for drawers (`translateX(110%)→0`), dialogs (`translateY+scale`), scrims (opacity).
- **Reduced-motion rule**: anything animating from `opacity:0` must still reach its visible state under `prefers-reduced-motion` / print; prefer transform-only entrances. Build this once as a shared motion layer so every component inherits it.
...
## Files
This handoff folder contains the three orientation docs below. **The prototype source it refers to lives at the project root** (the level above this folder) — every `Vyonix *.html` entry point, every `*.jsx` logic/data file, and `vy-app.css`. Keep this folder inside the project so the root-relative filenames in these docs resolve. Start here:
- `./BUILD_PROMPT.md` — paste-ready generator prompt for Claude Code (components + motion + execution order). **Read first.**
- `./README.md` — this reference spec (data model, tokens, motion, localStorage keys).
- `./PROJECT_ARCHITECTURE.md` — the complete living architecture doc (file map, data-flow bridges, change log).

Key source files at the project root:
