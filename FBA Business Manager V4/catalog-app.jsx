...
function CatFamilyRow({ family, onPeek, onOpenProduct }) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      onClick={() => onPeek(family.id)}
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onPeek(family.id); } }}
      style={{
        display: "flex", alignItems: "center", gap: 14, width: "100%", textAlign: "left",
        padding: "14px 16px", background: "transparent", border: "none", cursor: "pointer",
        borderTop: "1px solid hsl(var(--border) / 0.7)",
      }}
      className="vy-cat-row"
    >
      <CatThumb />
      <div style={{ flex: "1 1 260px", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <button
            type="button"
            className="vy-cat-title"
            onClick={(e) => { e.stopPropagation(); onOpenProduct(family.id); }}
            title="Open product page"
            style={{ background: "none", border: "none", padding: 0, margin: 0, font: "inherit", fontSize: 14, fontWeight: 700, color: "inherit", cursor: "pointer", textAlign: "left" }}
          >
      ...
      <div style={{ width: 110, textAlign: "right", flexShrink: 0 }}>
        <div style={{ fontSize: 13.5, fontWeight: 700, fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)", color: lowStock ? "hsl(var(--warning))" : undefined }}>
      ...
      <div style={{ width: 120, textAlign: "right", flexShrink: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 600, fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)" }}>{costLabel}</div>
        <div style={{ fontSize: 10.5, color: "hsl(var(--muted-fg))" }}>last unit cost</div>
      </div>
      <VyIcon name="chevronRight" size={15} style={{ opacity: 0.45, flexShrink: 0 }} />
    </div>
  );
}
          ...
          <div className="vy-content-inner">
            ...
            <div className="vy-card" style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px" }}>
                ...
                <span style={{ fontSize: 11, color: "hsl(var(--muted-fg))" }}>Sorted by family</span>
              </div>
              {filtered.length ? (
                filtered.map((f) => <CatFamilyRow key={f.id} family={f} onPeek={setPeekId} onOpenProduct={openProduct} />)
              ) : (
